...
**API 엔드포인트.** 모든 라우트는 [apps/api/src/index.ts](apps/api/src/index.ts)에서 `/api` 하위에 마운트: `/api/auth`(공개), 그리고 `/api/members`, `/api/memberships`, `/api/classes`, `/api/instructors`, `/api/instructor-schedules`, `/api/instructor-leaves`, `/api/reservations`, `/api/dashboard`, `/api/notifications`, `/api/assessments`(전부 `[requireAuth, requireRole('ADMIN')]` 적용 — Phase 2). 어드민은 [apps/admin/src/utils/api.ts](apps/admin/src/utils/api.ts)의 공통 `request<T>()` 헬퍼로 호출하며, 토큰은 [apps/admin/src/utils/auth.ts](apps/admin/src/utils/auth.ts)에서 `localStorage`로 관리하고 401이 떨어지면 자동으로 세션을 비우고 `/login`으로 보낸다. `VITE_API_URL` 기본값은 `http://localhost:3000`.
...
**i18n (Phase 5E).** 어드민은 `react-i18next` + `i18next-browser-languagedetector`로 한/영 토글. 초기화는 [src/i18n/index.ts](apps/admin/src/i18n/index.ts), 번역 키는 [locales/ko.json](apps/admin/src/i18n/locales/ko.json) / [en.json](apps/admin/src/i18n/locales/en.json) — 네임스페이스로 구성(`common`, `nav`, `auth`, `dashboard`, `charts`, `members`, `memberships`, `assessments`, `schedules`, `notifications`, `reservations`, `classes`, `instructors`, `modal`, `header`). Header에 [LanguageToggle](apps/admin/src/components/layout/LanguageToggle.tsx) — 클릭 시 ko↔en 순환, `localStorage` key `repoint.lang`로 영속화. fallback `ko`. **Enum-derived 표시(상태/타입/요일 등)는 raw value를 className으로 유지하면서 `t()` 로만 라벨 번역** — 예: `className={statusKey}` + `t('X.status.' + statusUpper)`. **백엔드가 발행하는 알림 title/body는 한국어 고정** — 사용자별 lang preference가 없어 발행 시점 언어가 적절. JSON import를 위해 `tsconfig.app.json`에 `resolveJsonModule: true` 활성화됨.

**라우팅.** `Sidebar` + `Header` + `<Outlet />` 구성의 단일 `MainLayout` 쉘 아래 `/`, `/members`, `/assessments`, `/memberships`, `/classes`, `/schedules`, `/reservations`, `/notifications` 페이지가 마운트. 전체 쉘은 [RequireAuth](apps/admin/src/components/auth/RequireAuth.tsx)로 감싸고, 공개 `/login`만 그 바깥. `/settings`는 플레이스홀더, 매칭되지 않는 경로는 `/`로 리다이렉트.
...
**신체 평가 (Phase 5A, minimal).** [`Assessment`](apps/api/prisma/schema.prisma) 는 PRD 3.1.4 의 **일부**만 표현 — `heightCm`, `weightKg`, `bmi`(자동), `bodyFatPct`, `muscleMassKg`, `notes`. **자세 분석 / 유연성 / 근력 / 운동 처방 / 사진 / PDF 리포트는 미도입**이라 PRD 측정 항목 30+ 개 중 5개만 다룬다는 점에 유의. BMI 는 [routes/assessments.ts](apps/api/src/routes/assessments.ts) 에서 height·weight 둘 다 있을 때 자동 계산되어 저장됨 — 클라이언트가 다시 계산하지 말 것. 어드민 [/assessments](apps/admin/src/pages/Assessments.tsx) 가 dual-axis line chart 로 추이를 표시.
